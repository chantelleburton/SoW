...
def get_era5_threshold(
    country: str, event_year: int, metric_stem: str, member: str = "1"
) -> float:
    """metric_stem: BaseMetric.output_stem(), e.g. 'FWI_P95', 'DSR_7X', 'DSR_CUM360_MEAN'."""
    path = os.path.join(
        METRICS_OUT_DIR, f"era5_{metric_stem}_{country}_{member}.csv"
    )
    ...
    if row.empty:
        raise ValueError(
            f"No ERA5 {metric_stem} value for {country} event_year={event_year} in {path}"
        )
