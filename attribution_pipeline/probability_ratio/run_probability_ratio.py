...
def run_risk_ratio(
    ...
    for country, res in results.items():
        likelihood = (
            (res["replicates"] >= 1).sum() / len(res["replicates"]) * 100
        )
        rows.append(
            {
                "Country": country,
...
def run_amplification(
    ...
    for country in countries:
        ...
        results[country] = compute_region_amplification(
            country, metric_stem, ensemble_folder, paired_only=paired_only
        )
...
if __name__ == "__main__":
    ...
    if not metric_stem:
        ...
        if os.environ.get("CYLC_TASK_PARAM_percentile"):
            metric_kwargs["percentile"] = os.environ[
                "CYLC_TASK_PARAM_percentile"
            ]
    ...
    ensemble_folder = os.environ.get(
        "CYLC_TASK_PARAM_ensemble_folder",
        os.path.join(BIAS_CORRECTED_BASE, historical_source),
    )
    bootstrap_size = int(
        os.environ.get("CYLC_TASK_PARAM_bootstrap_size", "10000")
    )
    paired_only = (
    ...
    countries = (
        countries_env.split(",") if countries_env else list(REGION_CONFIGS)
    )
    print(countries)
    if statistic == "risk_ratio":
        ...
        raise SystemExit(
            f"Unknown statistic {statistic!r}. Expected 'risk_ratio' or 'amplification'."
        )
