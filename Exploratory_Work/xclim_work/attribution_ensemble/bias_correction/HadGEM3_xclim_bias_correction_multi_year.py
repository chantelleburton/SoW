"""
Log-transform bias correction for the *xclim-derived* HadGEM3-A FWI attribution ensemble.

This is a direct adaptation of Bias_Correction/HadGEM3_LogBiasCorrection_MultiYear.py.
The bias-correction maths, baseline regression and logging are byte-for-byte the same;
ONLY the attribution-ensemble loader differs, because the xclim FWI is stored differently:

    Original (ImpactTB) attribution files
        dir  : /data/scratch/chantelle.burton/SoW2526/Y2526FWI/
        file : FWI_HadGEM3-A-N216_r{NNN}i1p{R}_{exp}_20230601-20250201_global_day.nc
        var  : 'canadian_fire_weather_index'  (name "Canadian Fire Weather Index")
        time : 2023-06-01 .. 2025-01-30  (only 2024 is a complete year)

    Xclim attribution files (this script)
        dir  : /data/scratch/bob.potts/sowf/Attribution_Ensemble_xclim/
        file : hadgem3a_fwi_{exp}_r{NNN}i1p{R}.nc
        var  : 'fwi'                          (name "Fire Weather Index"; single data var)
        time : 2019-11-01 .. 2024-12-30  (2020..2024 are complete years)

Everything else (grid, 360_day calendar, lat/lon/time coords, experiment tokens
historicalExt / historicalNatExt, 105 members x 5 realisations) is identical, so the
shapefile mask -> ConstrainToYear -> CountryPercentile -> TimePercentile pipeline and
the baseline CSVs are reused unchanged.

NOTE (scientific caveat): the baseline regression CSVs (HadGEM3_FWI_..._{member}_{pct}%.csv)
were generated from the ImpactTB FWI. Correcting xclim FWI against an ImpactTB-derived
simulated baseline mixes the two FWI implementations. For a fully self-consistent
correction the simulated baseline should also be regenerated from xclim FWI.
"""
...
# --- LOADER DIFFERENCE 1: attribution ensemble now comes from the xclim tree ---
attribution_folder = '/data/scratch/bob.potts/sowf/Attribution_Ensemble_xclim/Raw_FWI_Files/'
output_dir = '/data/scratch/bob.potts/sowf/Attribution_Ensemble_xclim/Condensed_Log_Transforms/'
baseline_folder = '/data/scratch/bob.potts/sowf/test_output/Baseline/'

os.makedirs(output_dir, exist_ok=True)

# --- LOADER DIFFERENCE 2: xclim files cover 2019-11 .. 2024-12, so 2020..2024 are ---
# complete years. This is the full set the original script was waiting on
# ("Currently set to just 2024 until 2020-2024 attribution ensemble runs are processed").
...
# Normalise Month to a tuple of 1-indexed month numbers for use with constrain_cube_to_months
months = Month if isinstance(Month, tuple) else (Month,)
print(f"Event month(s) for {Country}: {months} ({month})")
...
# --- LOADER DIFFERENCE 3: xclim files hold a single 'fwi' variable (name "Fire Weather
# Index"), not 'canadian_fire_weather_index'. Each file has one data var, so load_cube
# is called without a constraint name below.
...
def load_xclim_attribution_cube(folder, index_filestem, ensemble_member, realisation):
    """Xclim-specific loader (this is the ONLY substantive change vs the original).

    Returns (cube, filepath). Each xclim file holds a single 'fwi' variable, so we
    load without a variable-name constraint.
    """
    filename = f"hadgem3a_fwi_{index_filestem}_r{ensemble_member:03d}i1p{realisation}.nc"
    filepath = os.path.join(folder, filename)
    cube = iris.load_cube(filepath)
    return cube, filepath
...
for DATA_YEAR in DATA_YEARS:
    ...
    for e_idx, ensemble_member in enumerate(ensemble_members): # loop through all 105 ensemble members
        for r_idx, realisation in enumerate(realisations): #loop through physics realisations 1-5 for each of the 105 ensemble members
            ...
            # --- LOADER DIFFERENCE (filepath + variable) is isolated in this helper ---
            try:
                cube, filepath = load_xclim_attribution_cube(
                    attribution_folder, index_filestem, ensemble_member, realisation)
            except (IOError, OSError):
                filename = f"hadgem3a_fwi_{index_filestem}_r{ensemble_member:03d}i1p{realisation}.nc"
                missing.append((ensemble_member, realisation, os.path.join(attribution_folder, filename)))
                continue
    ...
    with open(log_file, 'w') as lf:
        ...
        lf.write(f"Source: xclim-derived FWI ({attribution_folder})\n")
